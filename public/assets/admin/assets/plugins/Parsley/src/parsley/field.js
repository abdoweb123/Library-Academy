...
Field.prototype = {
  ...
  validate: function (options) {
    ...
    if (!promise)  // If excluded with `groups` option
      return true;
  ...
  // or `undefined` if field is not in the given `groups`.
  whenValidate: function ({force, group} =  {}) {
    // do not validate a field if not the same as given validation groups
    this.refresh();
  ...
  isValid: function (options) {
    ...
    if (!promise) // Excluded via `groups`
      return true;
  ...
  // Just validate field. Do not trigger any event.
  // @returns a promise that succeeds only when all validations do
  // or `undefined` if the field is not in the given `groups`.
  ...
  whenValid: function ({force = false, value, group, _refreshed} = {}) {
    ...
    $.each(groupedConstraints, (_, constraints) => {
      // Process one groups of constraints at a time, we validate the constraints
      ...
      if (promise.state() === 'rejected')
        return false; // Interrupt processing if a groups has already failed
    });
