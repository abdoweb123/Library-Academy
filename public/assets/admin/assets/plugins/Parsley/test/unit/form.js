...
describe('Form', () => {
  ...
  it('should handle groups validation', () => {
    $('body').append(
      '<form id="element">'                                                                        +
        '<input id="field1" type="text" data-parsley-groups="foo" data-parsley-required="true" />'  +
        '<div id="field2"></div>'                                                                  +
    ...
    $('#field3').attr('data-parsley-groups', 'bar');
    expectWarning(() => {
      expect(parsleyForm.isValid('bar')).to.be(false);
    });
  });
  it('should handle groups validation with controls with multiple groups names', () => {
    $('body').append(
      '<form id="element">'                                                                        +
        '<input id="field1" type="text" data-parsley-groups=\'["foo", "bar"]\' data-parsley-required="true" />'  +
        '<input id="field2" type="text" data-parsley-groups=\'["bar", "baz"]\' data-parsley-required="true" />'  +
        '<textarea id="field3" data-parsley-groups=\'["baz", "qux"]\' data-parsley-required="true"></textarea>'  +
      '</form>');
    var parsleyForm = $('#element').parsley();
    expect(parsleyForm.isValid()).to.be(false);
    $('#field1').val('value');
    $('#field2').val('value');
    expect(parsleyForm.isValid()).to.be(false);
    // groups name only on one required field, with value
    expect(parsleyForm.isValid('foo')).to.be(true);
    // groups name on multiple required fields, all with values
    expect(parsleyForm.isValid('bar')).to.be(true);
    // groups name on multiple required fields, one missing a value
    expect(parsleyForm.isValid('baz')).to.be(false);
    // groups name on single required field, without value
