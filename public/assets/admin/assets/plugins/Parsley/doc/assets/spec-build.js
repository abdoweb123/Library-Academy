...
(function (global, factory) {
    ...
    parse: {
      ...
      regexp: function regexp(_regexp) {
        ...
        if (/^\/.*\/(?:[gimy]*)$/.test(_regexp)) {
          ...
          flags = _regexp.replace(/.*\/([gimy]*)$/, '$1'); // Again, replace the regexp literal string with the first match groups:
  ...
  Field.prototype = {
    ...
    whenValid: function whenValid() {
      ...
      if (!_refreshed) this.refresh(); // do not validate a field if not the same as given validation groups
      ...
      $.each(groupedConstraints, function (_, constraints) {
        ...
        if (promise.state() === 'rejected') return false; // Interrupt processing if a groups has already failed
      });
      return Utils.all(promises);
    },
    // @returns a promise
  ...
  describe('Form', () => {
    ...
    it('should handle groups validation', () => {
      $('body').append('<form id="element">' + '<input id="field1" type="text" data-parsley-groups="foo" data-parsley-required="true" />' + '<div id="field2"></div>' + '<textarea id="field3" data-parsley-required="true"></textarea>' + '</form>');
      var parsleyForm = $('#element').parsley();
    ...
    it('should handle groups validation with controls with multiple groups names', () => {
      $('body').append('<form id="element">' + '<input id="field1" type="text" data-parsley-groups=\'["foo", "bar"]\' data-parsley-required="true" />' + '<input id="field2" type="text" data-parsley-groups=\'["bar", "baz"]\' data-parsley-required="true" />' + '<textarea id="field3" data-parsley-groups=\'["baz", "qux"]\' data-parsley-required="true"></textarea>' + '</form>');
      var parsleyForm = $('#element').parsley();
      expect(parsleyForm.isValid()).to.be(false);
      $('#field1').val('value');
      $('#field2').val('value');
      expect(parsleyForm.isValid()).to.be(false); // groups name only on one required field, with value

      expect(parsleyForm.isValid('foo')).to.be(true); // groups name on multiple required fields, all with values

      expect(parsleyForm.isValid('bar')).to.be(true); // groups name on multiple required fields, one missing a value

      expect(parsleyForm.isValid('baz')).to.be(false); // groups name on single required field, without value
